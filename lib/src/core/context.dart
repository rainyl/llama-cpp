...
import 'package:llama_cpp/src/core/adapter.dart';
import 'package:llama_cpp/src/core/batch.dart';
import 'package:llama_cpp/src/core/enums.dart';
import 'package:llama_cpp/src/core/kv_cache.dart';
import 'package:llama_cpp/src/core/vocab.dart';
...
class ContextParams extends LLAMAStruct<llama.llama_context_params> {
  ...
  factory ContextParams.create() {
    final params = llama.llama_context_default_params();
    final p = calloc<llama.llama_context_params>()..ref = params;
...
//   external ffi.Pointer<ffi.Void> cb_eval_user_data;
  ...
  @override
  llama.llama_context_params get ref => ptr.ref;
}

class Context extends LLAMAClass<llama.llama_context> {
  static final finalizer = ffi.NativeFinalizer(llama.addresses.llama_free.cast());
  ...
  factory Context.fromModel(Model model, ContextParams params) {
    return Context(llama.llama_init_from_model(model.ptr, params.ref));
  }

  int get nCtx => llama.llama_n_ctx(ptr);

  int get nBatch => llama.llama_n_batch(ptr);

  int get nUbatch => llama.llama_n_ubatch(ptr);

  int get nSeqMax => llama.llama_n_seq_max(ptr);

  Model get model => Model(llama.llama_get_model(ptr)); // TODO: attach?

  PoolingType get poolingType => llama.llama_pooling_type(ptr);

  /// Add a loaded LoRA adapter to given context
  /// This will not modify model's weight
  int setAdapterLoRA(AdapterLoRA adapter, double scale) =>
      llama.llama_set_adapter_lora(ptr, adapter.ptr, scale);

  /// Remove a specific LoRA adapter from given context
  /// Return -1 if the adapter is not present in the context
  int rmAdapterLoRA(AdapterLoRA adapter) => llama.llama_rm_adapter_lora(ptr, adapter.ptr);

  /// Remove all LoRA adapters from given context
  void clearAdapterLoRA() => llama.llama_clear_adapter_lora(ptr);
  ...
  int applyAdapterCVec(Float32List data, int len, int nEmbd, int ilStart, int ilEnd) {
    return llama.llama_apply_adapter_cvec(ptr, data.address, len, nEmbd, ilStart, ilEnd);
  }

  /// Returns the number of tokens in the KV cache (slow, use only for debug)
  /// If a KV cell has multiple sequences assigned to it, it will be counted multiple times
  int get kvCacheTokenCount => llama.llama_get_kv_cache_token_count(ptr);

  /// Returns the number of used KV cells (i.e. have at least one sequence assigned to them)
  int get kvCacheUsedCells => llama.llama_get_kv_cache_used_cells(ptr);

  /// Clear the KV cache - both cell info is erased and KV data is zeroed
  void kvCacheClear() => llama.llama_kv_cache_clear(ptr);
  ...
  /// p0 < 0     : [0,  p1]
  /// p1 < 0     : [p0, inf)
  bool kvCacheSeqRM(int seqId, int p0, int p1) => llama.llama_kv_cache_seq_rm(ptr, seqId, p0, p1);
  ...
  void kvCacheSeqCP(int seqIdSrc, int seqIdDst, int p0, int p1) =>
      llama.llama_kv_cache_seq_cp(ptr, seqIdSrc, seqIdDst, p0, p1);

  /// Removes all tokens that do not belong to the specified sequence
  void kvCacheSeqKeep(int seqId) => llama.llama_kv_cache_seq_keep(ptr, seqId);
  ...
  void kvCacheSeqAdd(int seqId, int p0, int p1, int delta) =>
      llama.llama_kv_cache_seq_add(ptr, seqId, p0, p1, delta);
  ...
  /// p0 < 0 : [0,  p1]
  /// p1 < 0 : [p0, inf)
  void kvCacheSeqDiv(int seqId, int p0, int p1, int d) => llama.llama_kv_cache_seq_div(ptr, seqId, p0, p1, d);

  /// Returns the largest position present in the KV cache for the specified sequence
  int kvCacheSeqPosMax(int seqId) => llama.llama_kv_cache_seq_pos_max(ptr, seqId);

  /// Defragment the KV cache
  /// This will be applied:
  /// - lazily on next llama_decode()
  /// - explicitly with llama_kv_cache_update()
  void kvCacheDefrag() => llama.llama_kv_cache_defrag(ptr);

  /// Apply the KV cache updates (such as K-shifts, defragmentation, etc.)
  void kvCacheUpdate() => llama.llama_kv_cache_update(ptr);

  /// Check if the context supports KV cache shifting
  bool kvCacheCanShift() => llama.llama_kv_cache_can_shift(ptr);

  /// Returns the *actual* size in bytes of the state
  /// (logits, embedding and kv_cache)
  /// Only use when saving the state, not when restoring it, otherwise the size may be too small.
  int get stateSize => llama.llama_state_get_size(ptr);
  ...
  (int nBytes, Uint8List data) getState(int size) {
    ...
    final nBytes = llama.llama_state_get_data(ptr, p, size);
  ...
  int setState(Uint8List data) {
    final nBytes = llama.llama_state_set_data(ptr, data.address, data.length);
    return nBytes;
  }

  /// load session file
  (bool success, Int32List tokens) loadStateFromFile(String pathSession) {
    final cpath = pathSession.toNativeUtf8().cast<ffi.Char>();
    const nTokenCapacity = 1024;
    final pTokensOut = calloc<llama.llama_token>(nTokenCapacity);
    final pCount = calloc<ffi.Size>();
    final result = llama.llama_state_load_file(ptr, cpath, pTokensOut, nTokenCapacity, pCount);
  ...
  bool saveStateToFile(String pathSession, Int32List tokens) {
    final cpath = pathSession.toNativeUtf8().cast<ffi.Char>();
    final result = llama.llama_state_save_file(ptr, cpath, tokens.address, tokens.length);
    calloc.free(cpath);
    return result;
  }

  /// Get the exact size needed to copy the KV cache of a single sequence
  int getStateSeqSize(int seqId) => llama.llama_state_seq_get_size(ptr, seqId);

  /// Copy the KV cache of a single sequence into the specified buffer
  (int, Uint8List dst) getStateSeqData(int size, int seqId) {
    final p = calloc<ffi.Uint8>(size);
    final nBytes = llama.llama_state_seq_get_data(ptr, p, size, seqId);
    return (nBytes, p.asTypedList(size));
  }
  ...
  int setStateSeqData(Uint8List data, int seqId) {
    final result = llama.llama_state_seq_set_data(ptr, data.address, data.length, seqId);
    return result;
  }

  int saveStateSeqToFile(String filepath, int seqId, Int32List tokens) {
    final cpath = filepath.toNativeUtf8().cast<ffi.Char>();
    final result = llama.llama_state_seq_save_file(ptr, cpath, seqId, tokens.address, tokens.length);
    calloc.free(cpath);
    return result;
  }

  (int success, Int32List tokens) loadStateSeqFromFile(String filepath, int destSeqId) {
    final cpath = filepath.toNativeUtf8().cast<ffi.Char>();
    const nTokenCapacity = 1024;
    final pTokensOut = calloc<llama.llama_token>(nTokenCapacity);
    final pCount = calloc<ffi.Size>();
    final result = llama.llama_state_seq_load_file(ptr, cpath, destSeqId, pTokensOut, nTokenCapacity, pCount);
    final nTokenCount = pCount.value;
    calloc.free(cpath);
    calloc.free(pCount);
  ...
  Batch getOneBatch(Int32List tokens) {
    final p = llama.llama_batch_get_one(tokens.address, tokens.length);
  ...
  int encode(Batch batch) => llama.llama_encode(ptr, batch.ref);

  /// Positive return values does not mean a fatal error, but rather a warning.
  /// 0 - success
  /// 1 - could not find a KV slot for the batch (try reducing the size of the batch or increase the context)
  /// < 0 - error. the KV cache state is restored to the state before this call
  int decode(Batch batch) => llama.llama_decode(ptr, batch.ref);

  /// Set the number of threads used for decoding
  /// n_threads is the number of threads used for generation (single token)
  /// n_threads_batch is the number of threads used for prompt and batch processing (multiple tokens)
  void setNThreads(int nThreads, int nThreadsBatch) {
    llama.llama_set_n_threads(ptr, nThreads, nThreadsBatch);
  }

  /// Get the number of threads used for generation of a single token.
  int get nThreads => llama.llama_n_threads(ptr);

  /// Get the number of threads used for prompt and batch processing (multiple token).
  int get nThreadsBatch => llama.llama_n_threads_batch(ptr);

  /// Set whether the model is in embeddings mode or not
  /// If true, embeddings will be returned but logits will not
  set embeddings(bool embeddings) {
    llama.llama_set_embeddings(ptr, embeddings);
  }

  /// Set whether to use causal attention or not
  /// If set to true, the model will only attend to the past tokens
  set causalAttn(bool causalAttn) {
    llama.llama_set_causal_attn(ptr, causalAttn);
  }
  ...
  void synchronize() => llama.llama_synchronize(ptr);
  ...
  /// Cols: n_vocab
  ffi.Pointer<ffi.Float> getLogits() => llama.llama_get_logits(ptr);
  ...
  ffi.Pointer<ffi.Float> getLogitsIth(int i) {
    final p = llama.llama_get_logits_ith(ptr, i);
    return p;
  }
  ...
  //   ffi.Pointer<llama_context> ctx,
  // );
  ffi.Pointer<ffi.Float> getEmbeddings() => llama.llama_get_embeddings(ptr);
  ...
  ffi.Pointer<ffi.Float> getEmbeddingsIth(int i) {
    return llama.llama_get_embeddings_ith(ptr, i);
  }
  ...
  ffi.Pointer<ffi.Float> getEmbeddingsSeq(int seqId) {
    return llama.llama_get_embeddings_seq(ptr, seqId);
  }
